...
int get_student(int fd, int id, student_t *s) {
    off_t position = id * STUDENT_RECORD_SIZE;
    if (lseek(fd, position, SEEK_SET) == -1) {
        printf(M_ERR_DB_READ);
        return ERR_DB_FILE;
    }
    if (read(fd, s, STUDENT_RECORD_SIZE) != STUDENT_RECORD_SIZE || memcmp(s, &EMPTY_STUDENT_RECORD, STUDENT_RECORD_SIZE) == 0) {
...
int add_student(int fd, int id, char *fname, char *lname, int gpa) {
    ...
    if (get_student(fd, id, &(student_t){0}) == NO_ERROR) {
...
int del_student(int fd, int id) {
    ...
    if (lseek(fd, position, SEEK_SET) == -1 || write(fd, &EMPTY_STUDENT_RECORD, STUDENT_RECORD_SIZE) != STUDENT_RECORD_SIZE) {
        printf(M_ERR_DB_WRITE);
        return ERR_DB_FILE;
    }
...
int compress_db(int fd) {
    int new_fd = open(TMP_DB_FILE, O_RDWR | O_CREAT | O_TRUNC, 0644);
    if (new_fd == -1) {
    ...
    while (read(fd, &student, STUDENT_RECORD_SIZE) == STUDENT_RECORD_SIZE) {
        if (student.id != 0) write(new_fd, &student, STUDENT_RECORD_SIZE);
    }
    close(fd);
    close(new_fd);
